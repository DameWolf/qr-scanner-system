...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		// The Apps Script expects { action: 'sendQrCode', email, name }
		const response = await fetch(url, {
			method: 'POST',
			redirect: 'follow',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ action: 'sendQrCode', email, name })
		});
		...
		console.error('Failed to send QR/ID:', error);
		return json(
			{ success: false, message: 'Failed to send QR/ID' },
			{ status: 500 }
		);
	}
};
